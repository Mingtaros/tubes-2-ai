...
def isExistElementInErrorRange(li, num, eps):
...
class ShapeIdentifier(KnowledgeEngine):
    ...
    def segitiga_lancip(self, Fact):
        if (not [x for x in Fact['list_of_angles'] if (x >= 90-degree_epsilon)]): #tidak ada sudut yang >= 90 derajat
            self.result.append(("Segitiga Lancip", "(jumlah_sudut == 3) && (angles < 90) ==> Segitiga Lancip"))

    @Rule(AS.Fact << Fact(jumlah_sudut = L(3)))
    def segitiga_tumpul(self, Fact):
        if ([x for x in Fact['list_of_angles'] if (x > 90-degree_epsilon)]): #ada sudut yang > 90 derajat
    ...
    def segitiga_sikusiku(self, Fact):
        if (isExistElementInErrorRange(Fact['list_of_angles'], 90, degree_epsilon)):
    ...
    def segitiga_sama_kaki(self, Fact):
        ...
        if (len(Fact['list_of_angles']) == len(set(Fact['list_of_angles']))):
            ...
            elif (not [x for x in Fact['list_of_angles'] if (x >= 90-degree_epsilon)]): #tidak ada sudut yang >= 90 derajat
                self.result.append(("Segitiga Sama Kaki dan Lancip", "(jumlah_sudut == 3) && (angle_1 == angle_2) && (angles < 90) ==> Segitiga Sama Kaki dan Lancip"))

            elif ([x for x in Fact['list_of_angles'] if (x > 90-degree_epsilon)]): #ada sudut yang > 90 derajat
    ...
    def segitiga_sama_sisi(self, Fact):
        ...
        if (len(set([SUDUT_SEGITIGA_SAMA_SISI for x in Fact['list_of_angles'] if (x-degree_epsilon <= SUDUT_SEGITIGA_SAMA_SISI <= x+degree_epsilon)])) == 1 and isExistElementInErrorRange(Fact['list_of_angles'], SUDUT_SEGITIGA_SAMA_SISI, degree_epsilon)):
    ...
    def jajaran_genjang(self, Fact):
        if (isSame(Fact['list_of_angles'][0], Fact['list_of_angles'][2]) and isSame(Fact['list_of_angles'][1], Fact['list_of_angles'][3])):
            ...
            if (len(set([90 for x in Fact['list_of_angles'] if (x-degree_epsilon <= 90 <= x+degree_epsilon)])) == 1 and isExistElementInErrorRange(Fact['list_of_angles'], 90, degree_epsilon)):
    ...
    # wip untuk layang2
    @Rule(AS.Fact << Fact(jumlah_sudut = L(4)))
    def layang_layang(self, Fact):
        if (((isSame(Fact['list_of_angles'][0], Fact['list_of_angles'][2]) and (not isSame(Fact['list_of_angles'][1], Fact['list_of_angles'][3])))) or
            ((isSame(Fact['list_of_angles'][1], Fact['list_of_angles'][3]) and (not isSame(Fact['list_of_angles'][0], Fact['list_of_angles'][2])))) 
            and (not isExistElementInErrorRange(Fact['list_of_angles'], 90, degree_epsilon))):
    ...
    def segilima_sama_sisi(self, Fact):
        if (len(set([108 for x in Fact['list_of_angles'] if (x-degree_epsilon <= 108 <= x+degree_epsilon)])) == 1 and (108-degree_epsilon <= Fact['list_of_angles'][0] <= 108+degree_epsilon)):
